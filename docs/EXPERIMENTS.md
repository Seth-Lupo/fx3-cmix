...
**Schedule re-plan (2026-06-12 ~12:45 UTC):** T3 uncontended rate verdict 0.66%/h (no gain from uncontending) → lab2 T3 ETA ~Jun18 14:00. lab1 baseline SLOWING (0.97%/h current phase) → completes ~Jun14 evening, NOT Jun13 → relocation plan dead, lab2 = PRIMARY. NEW: reorder-T3 launched NOW on lab2 core2 (validity check that simply keeps running; size-only, timing-dirty, plain build ±~50B PGO offset) → reorder A/B answer ~Jun19 concurrent with primary. lab1 post-baseline (Jun14) = decode-RT + submission-build box. Budget lands ~$77/78 (user-approved overrun margin). Primary T3 wall-time now contention-dirty — official timing on submission build later.

**Reorder-T3 LAUNCHED (2026-06-12 12:50 UTC):** validity gate PASSED — our order file (md5 17a84055, 172,315 entries) accepted by the real transform, .main_reordered = 999,988,851 B exactly, compression underway on lab2 core2 (plain build ±~50B PGO offset, nice-10, size-only). A/B vs primary T3 (kaitz order, core0) answers "do 2026 embeddings beat doc2vec ordering" ~Jun18-19. Primary rate with dual load: 0.62%/h.

| v26-c RT | 2026-06-12 15:07 UTC | standalone v26 (fixed) | T3 | coded_full 586MB | **RT_FULL_OK — byte-exact round-trip** | — | ~16h decompress | — | ✅ **at-scale determinism PROVEN** | the reset-alignment fix holds at full production scale (the exact failure that killed kaitz's fx3); v26full lane CLOSED, lab1 core3 freed |

**S1 measured (submission-build, 2026-06-12 16:10 UTC): 455,757 B** = binary 155,756 (upx -9) + comp_dict 100,581 + comp_order 199,408 + header 12. vs fx2 our-build 441,983 / kaitz fx3 437,663 → **+18,094 vs kaitz = recoverable S1-diet target** (≈⅓ of the 50,431 gap). comp_order matches earlier measurement (new order is ~199.4K under cmix; delta-coding already rejected). Diet candidates: upx --ultra-brute (testing), -Os on more TUs, blob recompression tweaks. Packaged binary lives at lab1:/mnt/work/trees/subbuild/run/cmix.
